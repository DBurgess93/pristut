<script lang="ts">
	...
	import StyalizedLogoMark from './StyalizedLogoMark.svelte';
	...
	import IconDigitalOcaen from '~icons/fa6-brands/digital-ocean';

	const icons = {
		digitalocean: IconDigitalOcaen,
...
>
	...
	<div class="relative">
		<h2
			class="mx-auto max-w-2xl text-balance bg-gradient-to-b from-violet-50
      to-violet-300 bg-clip-text py-2 text-center text-5xl font-medium text-transparent md:text-7xl"
		>
		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.icons as item, index}
				{#if item.icon}
					...
					{/if}
					<div
						class="pulsing-icon flex aspect-square shrink-0 items-center justify-center
        rounded-full border border-violet-50/30 bg-violet-50/25 p-3 text-3xl text-violet-100 opacity-40 md:text-2xl lg:text-5xl"
					>
